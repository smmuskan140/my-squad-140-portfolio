...
export const BentoGrid = ({ children, className }: { children: ReactNode; className?: string }) => {
  return <div className={cn("grid w-full auto-rows-[22rem] grid-cols-1 gap-4 lg:grid-cols-3", className)}>{children}</div>;
};
...
export const BentoCard = ({
  ...
  <div
    className={cn(
      "group relative col-span-1 lg:col-span-3 flex flex-col justify-between overflow-hidden rounded-xl bg-white/90 text-neutral-900",
      "shadow-[0_2px_20px_rgba(0,0,0,0.08)] dark:bg-black dark:text-neutral-100 dark:[border:1px_solid_rgba(255,255,255,.1)]",
      className
    )}
  >
    <div>{background}</div>
